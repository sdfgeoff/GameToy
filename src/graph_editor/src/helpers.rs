...
pub fn list_edit<T: Clone, F>(
    ui: &mut Ui,
    item_list: &Vec<T>,
    mut draw_item_function: F,
    list_edit_id: &str,
) -> ListEditResponse
where
...
{
    let mut response = ListEditResponse::None;
    egui::Grid::new(list_edit_id).num_columns(2).show(ui, |ui| {
...
/// Allows editing of items within a list
pub fn list_edit_mut<T: Clone, F>(
    ui: &mut Ui,
    item_list: &mut Vec<T>,
    mut draw_item_function: F,
    list_edit_id: &str,
    ...
    egui::Grid::new(list_edit_id).num_columns(2).show(ui, |ui| {
        for (item_id, node) in reflist.iter().enumerate() {
            match list_edit_buttons(ui, item_id, item_list.len()) {
                ...
                ListEditResponse::Remove(item_id) => {
                    reflistout.remove(item_id);
                }
                ListEditResponse::Swap(item_id_1, item_id_2) => {
                    reflistout.swap(item_id_1, item_id_2)
                }
            }
