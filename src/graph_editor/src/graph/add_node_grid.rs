pub fn add_node_widget(node_list: &mut Vec<gametoy::config_file::Node>, ui: &mut egui::Ui) {
    ...
    if ui.button("RenderPass").clicked() {
        node_list.push(
            gametoy::config_file::Node::RenderPass(
                gametoy::config_file::RenderPassConfig {
                    name: format!("Render Pass {}", node_list.len()),
                    output_texture_slots: vec![
                        gametoy::config_file::OutputBufferConfig {
                            name: "RenderOut".to_string(),
                            format: gametoy::config_file::OutputBufferFormat::RGB8
                        }
                    ],
                    input_texture_slots: vec![
                        gametoy::config_file::InputBufferConfig{
                            name: "KeyboardInput".to_string()
                        }
                    ],
                    resolution_scaling_mode: gametoy::config_file::ResolutionScalingMode::ViewportScale(1.0, 1.0),
    ...
    if ui.button("Image").clicked() {
        node_list.push(
            gametoy::config_file::Node::Image(
    ...
    if ui.button("Keyboard").clicked() {
        node_list.push({
            gametoy::config_file::Node::Keyboard(
                gametoy::config_file::KeyboardConfig {
    ...
    if ui.button("Output").clicked() {
        node_list.push(
            gametoy::config_file::Node::Output(
