...
pub fn edit_render_pass(node: &mut RenderPassConfig, ui: &mut egui::Ui) {
    ...
    ui.vertical(|ui| {
        list_edit(
            ...
            |ui, _item_id, input_config| {
                egui::Grid::new("input_texture_slot_grid")
                    .num_columns(2)
                    .show(ui, |ui| {
                        ui.label("Name:");
    ...
    ui.vertical(|ui| {
        list_edit(
            ...
            |ui, _item_id, output_config| {
                egui::Grid::new("output_texture_slot_grid")
                    .num_columns(2)
                    .show(ui, |ui| {
                        ui.label("Name:");
...
pub fn resolution_scaling_mode_widget(scaling_mode: &mut ResolutionScalingMode, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        let mut current_scaling_mode =
            ResScalingModeUi::from_resolution_scaling_mode(&scaling_mode);
        egui::ComboBox::from_id_source(12345)
            .selected_text(current_scaling_mode.to_str())
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut current_scaling_mode,
                    ResScalingModeUi::Fixed,
                    ResScalingModeUi::Fixed.to_str(),
                );
                ui.selectable_value(
                    &mut current_scaling_mode,
                    ResScalingModeUi::ViewportScale,
                    ResScalingModeUi::ViewportScale.to_str(),
                );
            });
