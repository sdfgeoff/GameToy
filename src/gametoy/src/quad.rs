...
use glow::{Context, Buffer, HasContext, ARRAY_BUFFER, STATIC_DRAW, FLOAT, FALSE};
...
impl Quad {
    pub fn new(gl: &Context) -> Result<Self, QuadError> {
        let position_buffer = unsafe {
            upload_array_f32(gl, vec![-1.0, 1.0, 1.0, 1.0, 0.0, -1.0])?
        };
        Ok( Self {
            position_buffer,
        })
...
unsafe fn upload_array_f32(
    gl: &Context,
    vertices: Vec<f32>,
) -> Result<Buffer, QuadError> {

        let vao = gl.create_vertex_array().map_err(QuadError::BufferCreationFailed)?;
        gl.bind_vertex_array(Some(vao));
        let vbo = gl.create_buffer().map_err(QuadError::BufferCreationFailed)?;
